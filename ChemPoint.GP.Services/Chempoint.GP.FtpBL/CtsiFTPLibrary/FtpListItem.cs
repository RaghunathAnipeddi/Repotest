...
namespace Chempoint.GP.FtpBL.CtsiFTPLibrary
{
    ...
    public delegate FtpListItem Parser(string line, FtpCapabilities capabilities);
    ...
    public class FtpListItem : IFtpListItem {
        ...
        FtpPermissions m_ownerPermissions = FtpPermissions.None;
        /// <summary>
        /// Gets the owner permissions. This property can be
        /// set however this functionality is intended to be done by
        /// custom parsers.
        /// </summary>
        public FtpPermissions OwnerPermissions {
            get { 
        ...
        FtpPermissions m_groupPermissions = FtpPermissions.None;
        /// <summary>
        /// Gets the group permissions. This property can be
        /// set however this functionality is intended to be done by
        /// custom parsers.
        /// </summary>
        public FtpPermissions GroupPermissions {
            get { 
        ...
        FtpPermissions m_otherPermissions = FtpPermissions.None;
        /// <summary>
        /// Gets the others permissions. This property can be
        /// set however this functionality is intended to be done by
        /// custom parsers.
        /// </summary>
        public FtpPermissions OthersPermissions {
            get { 
        ...
        /// <param name="buffer">A line from the file listing</param>
        ...
        public static FtpListItem Parse(string path, string buffer, FtpCapabilities capabilities) {
            if (buffer != null && buffer.Length > 0) {
                FtpListItem item;

                foreach (Parser parser in Parsers) {
                    if ((item = parser(buffer, capabilities)) != null) {
        ...
        static FtpListItem ParseMachineList(string buf, FtpCapabilities capabilities) {
            ...
            if((m = Regex.Match(buf, @"unix.mode=(?<mode>\d+);", RegexOptions.IgnoreCase)).Success) {
                if (m.Groups["mode"].Value.Length == 4) {
                    ...
                    item.OwnerPermissions = (FtpPermissions)int.Parse(m.Groups["mode"].Value[1].ToString());
                    item.GroupPermissions = (FtpPermissions)int.Parse(m.Groups["mode"].Value[2].ToString());
                    item.OthersPermissions = (FtpPermissions)int.Parse(m.Groups["mode"].Value[3].ToString());
                }
                else if(m.Groups["mode"].Value.Length == 3) {
                    item.OwnerPermissions = (FtpPermissions)int.Parse(m.Groups["mode"].Value[0].ToString());
                    item.GroupPermissions = (FtpPermissions)int.Parse(m.Groups["mode"].Value[1].ToString());
                    item.OthersPermissions = (FtpPermissions)int.Parse(m.Groups["mode"].Value[2].ToString());
                }
            }
        ...
        /// <param name="capabilities">Server capabilities</param>
        /// <returns>FtpListItem if the item is able to be parsed</returns>
        static FtpListItem ParseUnixList(string buf, FtpCapabilities capabilities) {
            string regex = 
                @"(?<permissions>[\w-]{10})\s+" +
                @"(?<objectcount>\d+)\s+" +
                @"(?<user>[\w\d]+)\s+" +
            ...
            ////
            if ((!capabilities.HasFlag(FtpCapabilities.Mdtm) || item.Type == FtpFileSystemObjectType.Directory) && m.Groups["modify"].Value.Length > 0)
                ...
                if (perms.Success) {
                    if (perms.Groups["owner"].Value.Length == 3) {
                        if (perms.Groups["owner"].Value[0] == 'r')
                            item.OwnerPermissions |= FtpPermissions.Read;
                        if (perms.Groups["owner"].Value[1] == 'w')
                            item.OwnerPermissions |= FtpPermissions.Write;
                        if (perms.Groups["owner"].Value[2] == 'x' || perms.Groups["owner"].Value[2] == 's')
                            item.OwnerPermissions |= FtpPermissions.Execute;
                    ...
                    if (perms.Groups["group"].Value.Length == 3) {
                        if (perms.Groups["group"].Value[0] == 'r')
                            item.GroupPermissions |= FtpPermissions.Read;
                        if (perms.Groups["group"].Value[1] == 'w')
                            item.GroupPermissions |= FtpPermissions.Write;
                        if (perms.Groups["group"].Value[2] == 'x' || perms.Groups["group"].Value[2] == 's')
                            item.GroupPermissions |= FtpPermissions.Execute;
                    ...
                    if (perms.Groups["others"].Value.Length == 3) {
                        if (perms.Groups["others"].Value[0] == 'r')
                            item.OthersPermissions |= FtpPermissions.Read;
                        if (perms.Groups["others"].Value[1] == 'w')
                            item.OthersPermissions |= FtpPermissions.Write;
                        if (perms.Groups["others"].Value[2] == 'x' || perms.Groups["others"].Value[2] == 't')
                            item.OthersPermissions |= FtpPermissions.Execute;
        ...
        /// <param name="capabilities">Server capabilities</param>
        /// <returns>FtpListItem if the item is able to be parsed</returns>
        static FtpListItem ParseDosList(string buf, FtpCapabilities capabilities) {
            FtpListItem item = new FtpListItem();
        ...
        static FtpListItem ParseVaxList(string buf, FtpCapabilities capabilities) {
            string regex =
