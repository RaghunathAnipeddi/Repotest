...
namespace Chempoint.GP.FtpBL.CtsiFTPLibrary
{
    ...
    public delegate void FtpSslValidationEventHandler(FtpClient control, FtpSslValidationEventArgs e);
    ...
    /// FTPLibrary gets in return. In order to view this information
    /// you need to build FTPLibrary with DEBUG defined. When enable
    /// FTPLibrary will log to the System.Diagnostics.Debug trace
    ...
    /// FTPLibrary by getting a transaction log from the server. In
    /// order for this code to work FTPLibrary must be built with
    ...
    public class FtpClient : IDisposable, IFtpClient {
        ...
        FtpCapabilities m_caps = FtpCapabilities.None;
        /// <summary>
        /// Gets the server capabilties represented by flags
        /// </summary>
        public FtpCapabilities Capabilities {
            get {
        ...
        FtpSslValidationEventHandler m_sslvalidate = null;
        /// <summary>
        ...
        public event FtpSslValidationEventHandler ValidateCertificate {
            add {
                m_sslvalidate += value;
            }
            remove {
        ...
        void OnValidateCertficate(FtpSslValidationEventArgs e) {
            FtpSslValidationEventHandler evt;
        ...
        internal FtpClient CloneConnection() {
            ...
            conn.ValidateCertificate += new FtpSslValidationEventHandler(
        ...
        public virtual void Connect() {
            ...
            try {
                ...
                if (m_caps.HasFlag(FtpCapabilities.Utf8)) {
        ...
        /// <returns>An array of FtpListItem objects</returns>
        /// <example><code source="..\Examples\GetListing.cs" lang="cs" /></example>
        public FtpListItem[] GetListing(string path, FtpListOptions options) {
            ...
            try {
                ...
                if (!options.HasFlag(FtpListOptions.ForceList) && m_caps.HasFlag(FtpCapabilities.Mlsd))
                    listcmd = "MLSD";
                else {
                    if (options.HasFlag(FtpListOptions.NameList)) {
                        listcmd = "NLST";
                    }
                    else {
                        if (options.HasFlag(FtpListOptions.AllFiles))
                            listcmd = "LIST -a";
                        else
                            listcmd = "LIST";
                    }
                ...
                for (int i = 0; i < rawlisting.Count; i++) {
                    ...
                    if (item != null) {
                        if (options.HasFlag(FtpListOptions.Modify)) {
                            ...
                            if ((item.Modified == DateTime.MinValue || (listcmd.StartsWith("LIST") && item.Modified > DateTime.Now))
                                && m_caps.HasFlag(FtpCapabilities.Mdtm))
                                item.Modified = GetModifiedTime(item.FullName);
                        }

                        if (options.HasFlag(FtpListOptions.Size)) {
                            // if no size was parsed, the object is a file and the server
                            // supports the SIZE command, then load the file size
                            if (item.Size == -1) {
                                long size;

                                if (item.Type == FtpFileSystemObjectType.File && m_caps.HasFlag(FtpCapabilities.Size)
        ...
        public IAsyncResult BeginGetListing(string path, AsyncCallback callback, Object state) {
            return BeginGetListing(path, FtpListOptions.Modify | FtpListOptions.Size, callback, state);
        }


        delegate FtpListItem[] AsyncGetListing(string path, FtpListOptions options);
        ...
        /// <returns>IAsyncResult</returns>
        /// <example><code source="..\Examples\BeginGetListing.cs" lang="cs" /></example>
        public IAsyncResult BeginGetListing(string path, FtpListOptions options, AsyncCallback callback, Object state) {
            IAsyncResult ar;
        ...
        /// <example><code source="..\Examples\DeleteDirectory.cs" lang="cs" /></example>
        public void DeleteDirectory(string path, bool force, FtpListOptions options) {
            FtpReply reply;
        ...
        delegate void AsyncDeleteDirectory(string path, bool force, FtpListOptions options);
        ...
        /// <returns>IAsyncResult</returns>
        /// <example><code source="..\Examples\BeginDeleteDirectory.cs" lang="cs" /></example>
        public IAsyncResult BeginDeleteDirectory(string path, bool force, FtpListOptions options, AsyncCallback callback, object state) {
        ...
        /// <returns>True if the file exists</returns>
        /// <example><code source="..\Examples\FileExists.cs" lang="cs" /></example>
        public bool FileExists(string path, FtpListOptions options) {
        ...
        delegate bool AsyncFileExists(string path, FtpListOptions options);
        ...
        /// <returns>IAsyncResult</returns>
        /// <example><code source="..\Examples\BeginFileExists.cs" lang="cs" /></example>
        public IAsyncResult BeginFileExists(string path, FtpListOptions options, AsyncCallback callback, object state) {
        ...
        /// <param name="destination">The old or new full or relative path including the new name of the object</param>
        /// <example><code source="..\Examples\Rename.cs" lang="cs" /></example>
        public void Rename(string path, string destination) {
            FtpReply reply;
            ...
            try {
                ...
                if (!(reply = Execute("RNTO {0}", destination.GetFtpPath())).Success)
                    throw new FtpCommandException(reply);
            }
            finally {
                m_lock.ReleaseMutex();
        ...
        public IAsyncResult BeginRename(string path, string destination, AsyncCallback callback, object state) {
            AsyncRename func;
            IAsyncResult ar;

            ar = (func = new AsyncRename(Rename)).BeginInvoke(path, destination, callback, state);
            lock (m_asyncmethods) {
                m_asyncmethods.Add(ar, func);
            }
        ...
        public static FtpClient Connect(Uri uri, bool checkCertificate) {
            ...
            cl.ValidateCertificate += new FtpSslValidationEventHandler(delegate(FtpClient control, FtpSslValidationEventArgs e) {
